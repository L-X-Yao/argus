...
What it CAN'T automate (need user to do separately): WSL2/usbipd-win install on Windows (one-time), usbipd `bind`/`attach` of the FC (admin PowerShell), SSH key copy/generation, sudo password. The script will prompt clearly when it hits one.
