...
| 15 | WebSerial 指令支持 | ✅ᵗ | arm/disarm/mode/RTL/param_set/param_request_all + **mission_upload** (MISSION_COUNT→REQUEST→ITEM→ACK 状态机) + **log_list / log_download** (LOG_REQUEST_LIST→ENTRY 列表, LOG_REQUEST_DATA→DATA 流式下载, LOG_REQUEST_END 取消) 全部通过USB直连, 绕过Python后端 |
