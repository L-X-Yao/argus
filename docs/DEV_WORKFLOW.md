# Argus + PLKJ ArduPilot — Development Workflow
...
| Run Argus against the **real PLKJ FC** | **Windows** | `COM6/COM8` directly accessible; WebSerial and backend share the same host; no usbipd dance |
| Real-sensor calibration (compass/gyro/accel/level/baro) | **Windows** | Same as above |
| Flash FC firmware | **Windows** | PLKJ uploader / Mission Planner are Windows tools |
| End-user demo / acceptance | **Windows** | Matches the deployment environment |
| Edit PLKJ ArduPilot fork (C++) | **WSL2** | waf, SITL build, fork maintenance — Linux-first ecosystem |
...
## ArduPilot (PLKJ fork) — WSL only

```bash
# One-time
git clone --recursive git@github.com:penglikeji/ardupilot.git ~/ardupilot
cd ~/ardupilot
git checkout 179e491c8e                      # or pull PLKJ-master for latest
...
./waf configure --board <plkj-board-name>
./waf copter
# → build/<plkj-board>/bin/arducopter.apj
```
...
```
# In Windows Explorer
\\wsl.localhost\Ubuntu\home\administrator\ardupilot\build\<plkj-board>\bin\arducopter.apj
→ copy to C:\firmware\
→ flash with PLKJ uploader / Mission Planner over COM6
```
