...
class TestTileCache:
    ...
    def test_second_request_uses_cache(self, tile_cache_dir):
        ...
        with patch('backend.app.urlreq.urlopen') as mock_urlopen:
            ...
            # First request
            r1 = client.get('/api/tile/osm/5/10/15')
            assert r1.status_code == 200
            assert mock_urlopen.call_count == 1

            # Second request should use cache
            r2 = client.get('/api/tile/osm/5/10/15')
...
class TestTileBulkDownload:
    ...
    def test_bulk_download_zoom_clamped(self, tile_cache_dir):
        ...
        with patch('backend.app.urlreq.urlopen') as mock_urlopen:
            ...
            r = client.post('/api/tile_bulk_download', json={
                'lat_min': 30.0, 'lat_max': 30.001,
                'lon_min': 120.0, 'lon_max': 120.001,
                'z_min': -5, 'z_max': 25,
                'style': 'osm',
            })
            assert r.status_code == 200
