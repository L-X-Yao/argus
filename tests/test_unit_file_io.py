...
class TestJsonMission:
    ...
    def test_export_contains_required_fields(self):
        waypoints = [
            {'lat': 30.0, 'lon': 120.0, 'alt': 50, 'drop': False, 'delay': 0, 'speed': 0, 'type': 'wp', 'loiter_param': 0},
            {'lat': 30.1, 'lon': 120.1, 'alt': 60, 'drop': True, 'delay': 5, 'speed': 3, 'type': 'spline', 'loiter_param': 0},
        ]
    ...
    def test_export_preserves_all_waypoint_fields(self):
        wp = {'lat': 30.5, 'lon': 120.5, 'alt': 75, 'drop': True, 'delay': 3, 'speed': 5, 'type': 'spline', 'loiter_param': 2}
...
class TestCsvLogExport:
    ...
    EXPECTED_HEADERS = [
        'time', 'roll', 'pitch', 'yaw', 'lat', 'lon', 'alt_rel', 'alt_msl',
        'gs', 'vz', 'voltage', 'current', 'remaining', 'mode', 'mode_name',
        'armed', 'gps_fix', 'sats', 'wp', 'hdg', 'dist', 'bat_time',
    ]
    ...
    def _generate_csv_line(self) -> str:
        ...
        return '%.2f,%.2f,%.2f,%.1f,%.7f,%.7f,%.1f,%.1f,%.1f,%.1f,%.2f,%.2f,%d,%d,%s,%d,%d,%d,%d,%.0f,%.0f,%d' % (
            10.5, 5.2, -3.1, 180.0, 30.1234567, 120.1234567, 15.3, 115.3,
            5.5, -0.3, 12.45, 8.32, 75, 4, 'GUIDED', 1, 3, 12, 3, 180, 250, 600
        )
