...
<div class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm">
  <div class="bg-card border border-border rounded-xl shadow-2xl p-6 w-[360px] max-w-[90vw]"
       role="dialog" aria-modal="true" aria-labelledby="auth-title" tabindex="-1">
    ...
    <label for="auth-token-input" class="block text-xs font-medium text-muted-foreground mb-1.5">{t('auth.tokenLabel')}</label>
    <input id="auth-token-input" bind:this={inputEl} bind:value={token} type="password"
           ...
           onkeydown={onKey} disabled={loading} />
    {#if error}
