<script lang="ts">
  ...
  const steps: Step[] = [
    { key: 'setup.frame',    icon: Layers,     color: 'text-blue-500',   callback: () => onframe() },
    { key: 'setup.rc',       icon: Radio,      color: 'text-orange-500', callback: () => onrccal() },
    { key: 'setup.motor',    icon: Zap,        color: 'text-yellow-500', callback: () => onmotor() },
    { key: 'setup.cal',      icon: Compass,    color: 'text-green-500',  callback: () => oncalibration() },
    { key: 'setup.failsafe', icon: ShieldAlert, color: 'text-red-500',   callback: () => onfailsafe() },
    { key: 'setup.modes',    icon: Plane,      color: 'text-purple-500', callback: () => onmodes() },
  ];
      ...
      <div class="flex items-center justify-center gap-1 mb-6 flex-wrap">
        {#each steps as s, i}
          <div class="flex items-center gap-1">
            <!-- svelte-ignore a11y_no_static_element_interactions -->
            <!-- svelte-ignore a11y_click_events_have_key_events -->
            <div class="flex items-center gap-1 cursor-pointer" onclick={() => goToStep(i)}>
