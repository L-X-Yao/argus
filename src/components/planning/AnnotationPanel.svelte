...
<div role="presentation" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center" onclick={onclose}>
    <div role="presentation" class="bg-card border border-border rounded-2xl overflow-hidden w-[450px] max-h-[80vh] shadow-2xl flex flex-col" onclick={(e) => e.stopPropagation()}>
      ...
      {:else}
        <div class="bg-muted/30 rounded-lg p-3 space-y-2">
          <div class="flex items-center gap-2">
            ...
            <input id="ann-name" type="text" bind:value={formName} placeholder="..."
          ...
          <div class="flex items-center gap-2">
            ...
            <input id="ann-note" type="text" bind:value={formNote} placeholder="..."
                   class="flex-1 h-7 px-2 text-xs bg-input border border-border rounded-md text-foreground focus:outline-none focus:ring-1 focus:ring-ring/50" />
          </div>
          <div class="flex items-center gap-2">
            <label for="ann-lat" class="text-xs text-muted-foreground w-12 shrink-0">Lat</label>
            <input id="ann-lat" type="number" step="0.000001" bind:value={formLat}
                   class="flex-1 h-7 px-2 text-xs bg-input border border-border rounded-md text-foreground focus:outline-none focus:ring-1 focus:ring-ring/50" />
          </div>
          <div class="flex items-center gap-2">
            <label for="ann-lon" class="text-xs text-muted-foreground w-12 shrink-0">Lon</label>
            <input id="ann-lon" type="number" step="0.000001" bind:value={formLon}
                   class="flex-1 h-7 px-2 text-xs bg-input border border-border rounded-md text-foreground focus:outline-none focus:ring-1 focus:ring-ring/50" />
          </div>
