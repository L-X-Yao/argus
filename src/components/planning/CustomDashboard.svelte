...
<div role="dialog" aria-modal="true" tabindex="-1" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center" onclick={onclose} onkeydown={(e) => { if (e.key === "Escape") onclose(); }}>
    <div role="presentation" class="bg-card border border-border rounded-2xl overflow-hidden w-[600px] max-h-[85vh] shadow-2xl flex flex-col" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()}>
    <!-- Header -->
      ...
      {:else}
        <div class="grid grid-cols-3 gap-2.5">
          {#each activeWidgets as w}
            ...
            <div class="bg-muted/20 border {statusBorder[st]} rounded-xl p-3 flex flex-col items-center justify-center min-h-[80px] transition-colors">
              <div class="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1">{t(w.labelKey)}</div>
